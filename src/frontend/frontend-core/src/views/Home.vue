    ...
    <header class="header">
      ...
      <div class="header-center">
        <div class="search-bar">
          <span class="search-label">美妆分类</span>
          ...
          <select class="filter-select"v-model="selectedBrand">
            ...
            <option value>兰蔻 (Lancome)</option>
            <option value>圣罗兰 (YSL)</option>
            <option value>巴黎欧莱雅 (L'Oreal Paris)</option>
      ...
      <section class="new-products">
        <div class="section-title">
          ...
          <div class="carousel-controls">
            <button class="control-btn" @click="prevProduct">上一张</button>
            <button class="control-btn" @click="nextProduct">下一张</button>
          </div>
        </div>
...
const normalizeBanner = (item) => ({
  ...
  btnText: item.btn_text || item.btnText || '立即查看',
  link: item.link || '#',
});
...
const normalizeProduct = (item) => ({
  ...
  image: getFullImageUrl(item.cover_image || item.cover || item.image),
...
const fetchHomeData = async () => {
  ...
  try {
    ...
    banners.value = (Array.isArray(bannersRes) ? bannersRes : []).map(normalizeBanner);
    newProducts.value = (Array.isArray(newProductsRes) ? newProductsRes : []).map(normalizeProduct);
...
.carousel {
  ...
  border-radius: 12px; /* 增加圆角，和整体风格统一 */
}
.carousel-item {
  position: absolute;
  top: 0;
...
.carousel-text {
  ...
  left: 40px;
  color: #fff;
  text-shadow: 0 2px 8px rgba(0, 0, 0, 0.4); /* 增加阴影，文字更清晰 */
}
.carousel-text h3 {
  font-size: 26px;
  margin-bottom: 8px;
...
.section-title {
  ...
  margin-bottom: 20px;
  padding-bottom: 10px;
  border-bottom: 1px solid #ffe6ef; /* 增加底部分割线，排版更清晰 */
}
.heart {
  color: #ff69b4;
  font-size: 20px;
