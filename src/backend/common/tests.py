...
from common.models import SupportMessage
...
class SupportMessageApiTests(APITestCase):
    def test_submit_message_success(self):
        ...
        self.assertEqual(response.data["data"]["nickname"], "Guest A")
        self.assertEqual(response.data["data"]["content"], "When will this order ship?")
        self.assertFalse(response.data["data"]["is_replied"])

    def test_reply_requires_auth(self):
        message = SupportMessage.objects.create(nickname="Anonymous", content="Pending reply")

        response = self.client.patch(
            f"/api/support/messages/{message.id}/reply/",
            {"reply_content": "Received"},
            format="json",
        )
    ...
    def test_staff_reply_success_and_list_visible(self):
        ...
        reply_response = self.client.patch(
            f"/api/support/messages/{message.id}/reply/",
            {"reply_content": "Hello, this will ship today."},
            format="json",
        )
        ...
        self.assertEqual(reply_response.data["data"]["reply_content"], "Hello, this will ship today.")
